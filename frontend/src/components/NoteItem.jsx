...
const createSnippet = (content, length) => content.length > length ? content.substring(0, length) + "..." : content;
export const NoteItem = memo(({note, onClick, onDelete}) =>
{
    ...
    return (
        <li className="note-item" style={{backgroundColor: note.color}}
            onClick={() => onClick(note.id)} role="button" tabIndex={0}
